...
export const UserVerification = () => {
  ...
  const handleSubmit = async (verificationCode: string) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message?.includes('Incorrect code')) {
  ...
  return (
    <div className="relative overflow-hidden flex flex-col items-center justify-center h-screen w-screen bg-black">
      ...
      >
        <p className="text-gray-300 font-bold dark:text-gray-400 text-2xl text-center px-4">
          Please enter the verification code sent to → <br />
          <span className="text-gray-300">{location.state.email}</span>
        </p>
        ...
        <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 space-x-0 md:space-x-4">
          ...
          >
            ...
            <span className="text-sm text-neutral-700 dark:text-neutral-300 font-bold">Submit</span>
            <BottomGradient />
          </button>
          ...
          <button
            ...
            onClick={async () => {
              ...
              await signUp.signUp?.prepareVerification({
                strategy: 'email_code',
              });
