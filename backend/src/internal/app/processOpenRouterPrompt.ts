...
import { isNullOrUndefined, notNullOrUndefined } from "../../pkg/data-utils"
...
export const processUserPrompt = async (prompt: string) => {
                ...
                "features": [
                  {
                    ...
                    "cost_analysis": {
                      ...
                      "development time in days": "Relative effort of development in days",
                      "maintenance OPEX Score": "Opex score (1-5)"
                    },
                    "scalability": {
  ...
  if(notNullOrUndefined(response) && !isEmpty(response)) {
   return response as unknown as ISystemArchitecture 

  }else{
    return 'Error Generating Response using Open Router API'
  }
...
export const processUserPromptStream = async (
  prompt: string, 
  onChunk: (chunk: string) => void,
  onComplete: (fullResponse: ISystemArchitecture) => void
) => {
  logger.info(`System Design Prompt Received (Streaming): ${prompt}`);
  

    
    const systemDesignPrompt = `You are floww, the best chief technology officer existing in the world. You combine expert-level coding skills with deep system design knowledge. You will receive tasks from the user and your mission is to accomplish them using the tools at your disposal while following these guidelines:
        ...
        try {
          ...
          const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
            ...
            body: JSON.stringify({
              ...
              messages: [{ role: 'user', content: systemDesignPrompt }],
              stream: true,
              temperature: 0.5
            }),
          });
          ...
          if (!response.ok) {
            throw new Error(`OpenRouter API streaming call failed with status: ${response.status}`);
          }
              ...
              while (true) {
                ...
                if (line.startsWith('data: ')) {
                  ...
                  try {
                    ...
                    logger.debug(`Ignoring invalid JSON: ${line}`);
                  }
                }
              }
            }
            
            logger.info(`Design Specifications for ${prompt} generated (streaming) !!!`);
          ...
          const errorMessage = error instanceof Error ? error.message : String(error);
          logger.error(`Error processing user prompt (streaming): ${errorMessage}`);
          throw error;
        }
      };